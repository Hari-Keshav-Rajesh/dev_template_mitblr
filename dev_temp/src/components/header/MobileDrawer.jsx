...
import Link from "next/link"
  ...
  export function MobileDrawer() {
    ...
    const icon = <svg width="30" height="30" viewBox="0 0 15 15" fill='CurrentColor' xmlns="http://www.w3.org/2000/svg"><path d="M1.5 3C1.22386 3 1 3.22386 1 3.5C1 3.77614 1.22386 4 1.5 4H13.5C13.7761 4 14 3.77614 14 3.5C14 3.22386 13.7761 3 13.5 3H1.5ZM1 7.5C1 7.22386 1.22386 7 1.5 7H13.5C13.7761 7 14 7.22386 14 7.5C14 7.77614 13.7761 8 13.5 8H1.5C1.22386 8 1 7.77614 1 7.5ZM1 11.5C1 11.2239 1.22386 11 1.5 11H13.5C13.7761 11 14 11.2239 14 11.5C14 11.7761 13.7761 12 13.5 12H1.5C1.22386 12 1 11.7761 1 11.5Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
  ...
  const toggleDrawer = (inOpen) => (event) => {
    ...
    return (
        <Box sx={{ display: 'flex' }}>
          <Button variant="outlined" color="" onClick={toggleDrawer(true)}>
            {icon}
          </Button>
          <Drawer anchor='right' open={open} onClose={toggleDrawer(false)} className="backdrop-blur">
            <Box
              role="presentation"
              onClick={toggleDrawer(false)}
              onKeyDown={toggleDrawer(false)}
              className="h-screen bg-slate-300 dark:bg-blue-800"
            >
            <div className='flex flex-col gap-28 mt-36'>
            <div
			className="transition duration-150 hover:opacity-70 flex justify-center items-center "
		    >
            <Link href="/">
                <svg
                    version="1.0"
              ...
              <div className="flex gap-10 flex-col content-center justify-center text-center">
                <Link href="/Events"><button className="font-bold text-4xl text-gray-900 dark:text-slate-200">Events</button></Link>
